...
Strategy_autopin1::Strategy_autopin1(Configuration *config, ObservedProcess *proc, OSServices *service,
...
Configuration::configopts Strategy_autopin1::getConfigOpts() {
...
void Strategy_autopin1::slot_autopinReady() {
	context.enableIndentation();
...
void Strategy_autopin1::slot_startPinning() {
...
void Strategy_autopin1::slot_startMonitor() {
	context.enableIndentation();
...
void Strategy_autopin1::slot_stopPinning() {
	context.enableIndentation();
	notifications = false;
...
void Strategy_autopin1::slot_TaskCreated(int tid) {
...
void Strategy_autopin1::slot_TaskTerminated(int tid) {
	if (notifications) {
		auto it = std::find_if(pinned_tasks.begin(), pinned_tasks.end(), [tid](const pinned_task &t) {
		   return t.tid == tid;
		});
...
void Strategy_autopin1::slot_PhaseChanged(int newphase) {
...
void Strategy_autopin1::applyPinning(PinningHistory::autopin_pinning pinning) {
...
void Strategy_autopin1::checkPinnedTasks() {
