...
Strategy_history::Strategy_history(Configuration *config, ObservedProcess *proc, OSServices *service,
...
Configuration::configopts Strategy_history::getConfigOpts() {
...
void Strategy_history::slot_autopinReady() {
	context.enableIndentation();
...
void Strategy_history::slot_startPinning() {
...
void Strategy_history::slot_TaskCreated(int tid) {
...
void Strategy_history::slot_TaskTerminated(int tid) {
	if (notifications) {
		auto it = std::find_if(pinned_tasks.begin(), pinned_tasks.end(), [tid](const pinned_task& t) {
			return t.tid == tid;
		});
...
void Strategy_history::slot_PhaseChanged(int newphase) {
...
void Strategy_history::applyPinning(PinningHistory::autopin_pinning pinning) {
...
void Strategy_history::checkPinnedTasks() {
