...
#include "Monitor_random.h"

#include <stdlib.h>
#include <QTime>

Monitor_random::Monitor_random(QString name, Configuration *config, const AutopinContext &context)
	: PerformanceMonitor(name, config, context) {
	type = "random";
}

void Monitor_random::init() {
...
Configuration::configopts Monitor_random::getConfigOpts() {
...
void Monitor_random::start(int tid) { rands[tid] = getRandomValue(); }

double Monitor_random::value(int tid) {
...
double Monitor_random::stop(int tid) {
	double result;
...
void Monitor_random::clear(int tid) {
	if (rands.find(tid) != rands.end()) rands.erase(tid);
}

ProcessTree::autopin_tid_list Monitor_random::getMonitoredTasks() {
...
double Monitor_random::getRandomValue() {
	double result = qrand();
